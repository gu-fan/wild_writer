...
func _ready():
    ...
    label.scale = Vector2(2, 2) * extra_scale
    label2.scale = Vector2(2, 2) * extra_scale
...
func _update_color():
    ...
    label3.modulate = Color(current_color).lightened(0.2)
...
func color_label():
    ...
    TwnLite.at(label, true, '_twn_scale').tween({
        prop='scale',
        from = Vector2(1.6,1.6) * randf_range(1.05, 1.1) * get_count_facor_scale(extra_scale), 
        to = Vector2(2, 2) * extra_scale,
    ...
    TwnLite.at(label2).tween({
        ...
        prop='scale',
        from = Vector2(1.6,1.6) * 1.1 *  get_count_facor_scale(extra_scale),
        to = Vector2(2, 2) * extra_scale,
